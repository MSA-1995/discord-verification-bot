...
AZKAR_INTERVAL_MINUTES = max(1, int(os.getenv("AZKAR_INTERVAL_MINUTES", "30")))
...
class AzkarSystem(commands.Cog):
    ...
    async def _get_or_create_azkar_channel(self, guild):
        channel = channels_config.get_azkar_channel(guild)
        if channel:
            return channel

        me = guild.me or guild.get_member(self.bot.user.id)
        if not me or not me.guild_permissions.manage_channels:
            print(f"⚠️ Missing Manage Channels permission to create {channels_config.AZKAR_CHANNEL_NAME} in {guild.name}.")
            return None

        try:
            return await guild.create_text_channel(
                name=channels_config.AZKAR_CHANNEL_NAME,
                reason="Azkar channel - ᴹˢᴬ",
            )
        except discord.Forbidden:
            print(f"⚠️ Forbidden: cannot create {channels_config.AZKAR_CHANNEL_NAME} in {guild.name}.")
        except discord.HTTPException as e:
            print(f"⚠️ Failed to create {channels_config.AZKAR_CHANNEL_NAME} in {guild.name}: {e}")
        return None
