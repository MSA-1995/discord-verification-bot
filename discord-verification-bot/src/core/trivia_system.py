...
class TriviaSystem(commands.Cog):
    ...
    async def _get_or_create_game_channel(self, guild: discord.Guild) -> discord.TextChannel | None:
        channel = channels_config.get_game_channel(guild)
        if channel:
            return channel
        try:
            channel = await guild.create_text_channel(
                name=channels_config.GAME_CHANNEL_NAME,
                reason="Trivia game channel",
            )
            _logger.info("trivia: created #%s in %s", channels_config.GAME_CHANNEL_NAME, guild.name)
            return channel
        except discord.Forbidden:
            _logger.warning("trivia: no permission to create #%s in %s", channels_config.GAME_CHANNEL_NAME, guild.name)
    ...
    async def on_message(self, message: discord.Message):
        ...
        elif content == "انتهى" and message.channel.name == channels_config.GAME_CHANNEL_NAME:
