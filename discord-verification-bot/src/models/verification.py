...
class Verification(commands.Cog):
    ...
    async def start_captcha(self, member: discord.Member, guild: discord.Guild, interaction: discord.Interaction = None):
        ...
        try:
            ...
            category = guild.get_channel(channels_config.VERIFY_CATEGORY_ID)
            if not category:
            ...
            for attempt in range(1, CAPTCHA_MAX_TRIES + 1):
                ...
                # وقت ديناميكي حقيقي: ديسكورد يعرض عد تنازلي حي على جهاز العضو تلقائياً
                deadline_ts = int((discord.utils.utcnow() + timedelta(seconds=CAPTCHA_TIMEOUT)).timestamp())

                embed = discord.Embed(
                    title="🔐 التحقق من الهوية",
                    description=(
                        f"أجب على السؤال التالي لإتمام التوثيق:\n\n"
                        f"**كم ناتج: {question} ؟**\n\n"
                ...
                # المنشن هنا بمحتوى الرسالة (content) وليس داخل الـ Embed، عشان يوصل إشعار فعلي للعضو
