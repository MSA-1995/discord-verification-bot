...
def get_invite_create_action(member):
    if member is None:
        return "log_unknown"
...
class Protection(commands.Cog):
    ...
    async def on_invite_create(self, invite):
        ...
        action = get_invite_create_action(member)
        ...
        if action == "delete_and_log":
            title = "باند | إنشاء دعوة غير مصرح"
        ...
        if inviter:
            fields.extend([
                ("الشخص", inviter.mention, True),
                ("الآيدي", f"`{inviter.id}`", True),
            ])
        else:
            fields.append(("الشخص", "غير معروف", True))

        fields.extend([
            ("الروم", invite.channel.mention if invite.channel else "غير معروف", True),
            ("الدعوة", getattr(invite, "url", f"https://discord.gg/{invite.code}"), False),
            ("الإجراء", "حذف الدعوة" if action == "delete_and_log" else "تسجيل فقط", True),
        ])
