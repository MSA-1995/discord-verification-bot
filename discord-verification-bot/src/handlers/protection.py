...
class Protection(commands.Cog):
    ...
    async def _setup_honeypot_message(self, guild: discord.Guild):
        honeypot_id = os.getenv("HONEYPOT_CHANNEL_ID", "1522303725316603944")
        channel = guild.get_channel(int(honeypot_id)) if honeypot_id.isdigit() else None
        if not channel:
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent):
        ...
        honeypot_id = os.getenv("HONEYPOT_CHANNEL_ID", "1522303725316603944")
        is_honeypot = (honeypot_id.isdigit() and channel.id == int(honeypot_id)) or channel.name == "تحذير"
        if not is_honeypot:
            return
    ...
    async def on_message(self, message):
        ...
        honeypot_id = os.getenv("HONEYPOT_CHANNEL_ID", "1522303725316603944")
        honeypot_channel = message.guild.get_channel(int(honeypot_id)) if honeypot_id.isdigit() else None
