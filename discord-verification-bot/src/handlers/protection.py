...
class Protection(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith("!") or message.content.startswith("/"):
            if message.author.id != message.guild.owner_id:
                try:
                    await message.delete()
                    banned = await self._queue_ban(member, "🚫 Unauthorized command usage - auto ban")
                    ...
                    if banned:
                        ...
                        embed = self._build_ban_failed_embed(
                            member, "استخدام أوامر",
                            f"كتب أمر: `{message.content[:200]}`",
                            channel=message.channel
                        )
                    await self.send_security_log(message.guild, embed)
                except (discord.Forbidden, discord.HTTPException) as e:
    ...
    # =====================================================
    # on_invite_create - تحديد الدعوات بشخص واحد لغير الأونر
    # =====================================================
    @commands.Cog.listener()
    async def on_invite_create(self, invite):
        if invite.inviter.id == invite.guild.owner_id:
            return
        ...
        try:
            await invite.delete(reason="🚫 Invite not limited to 1 use")
            new_invite = await invite.channel.create_invite(
                max_uses=1,
                max_age=invite.max_age,
                reason=f"دعوة محدودة بشخص واحد - أنشأها {invite.inviter}"
            )
            try:
                await invite.inviter.send(
                    f"✅ تم تحديد دعوتك بشخص واحد فقط.\n🔗 الرابط الجديد: {new_invite.url}"
                )
            except discord.Forbidden:
                pass
        except (discord.Forbidden, discord.HTTPException) as e:
            logger.error("on_invite_create limit error: %s", e)
