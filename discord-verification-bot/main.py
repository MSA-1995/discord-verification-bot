...
class MSABot(commands.Bot):
    ...
    async def _find_lease_message(self, channel):
        if self._lease_message_id:
            try:
                ...
                if message.author.id == self.user.id and message.content.startswith(LEASE_MARKER):
                    return message
    ...
    def _read_lease_payload(self, message):
        if not message or not message.content.startswith(LEASE_MARKER):
            return {}
    ...
    async def _sync_singleton_lease(self, channel):
        ...
        content = f"{LEASE_MARKER} {json.dumps(lease_payload, separators=(',', ':'))}"
