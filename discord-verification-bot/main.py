...
LEASE_CHANNEL_NAME = channels_config.SINGLETON_CHANNEL_NAME
...
class ᴹˢᴬBot(commands.Bot):
    ...
    async def _get_alert_session(self) -> aiohttp.ClientSession:
        ...
        return self._alert_session

    async def send_critical_alert(self, error_type, message, details=None):
        if not CRITICAL_WEBHOOK:
            return
        fields = [
            {"name": "Bot",        "value": "ᴹˢᴬ Core",                          "inline": True},
        ...
        embed = {
            ...
            "color": 0xff0000,
            "fields": fields,
            "footer": {"text": "ᴹˢᴬ Core • System Alerts"},
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
        try:
            session = await self._get_alert_session()
            await session.post(CRITICAL_WEBHOOK, json={"embeds": [embed]},
                               timeout=aiohttp.ClientTimeout(total=5))
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            _logger.debug("send_critical_alert failed: %s", e)

    async def setup_hook(self):
        print("🔄 setup_hook started...")

        if self._alert_session and not self._alert_session.closed:
            await self._alert_session.close()
            self._alert_session = None
        ...
        extensions = [
            ...
            'src.core.log_system',
            'src.core.azkar_system',
            'src.core.trivia_system',
            'src.handlers.backup',
            'src.handlers.extras'
        ]
    ...
    async def _sync_bot_role_names(self):
        for guild in self.guilds:
            ...
            bot_roles = [
                role for role in me.roles
                if role.managed and not role.is_default()
            ]
            if not bot_roles:
                continue

            bot_role = max(bot_roles, key=lambda role: role.position)
            if bot_role.name == channels_config.BOT_ROLE_NAME:
                continue

            try:
                await bot_role.edit(
                    name=channels_config.BOT_ROLE_NAME,
                    reason="توحيد اسم رتبة البوت",
                )
                print(f"✅ Bot role renamed to {channels_config.BOT_ROLE_NAME} in {guild.name}")
            except discord.Forbidden:
                print(f"⚠️ Missing permission to rename bot role in {guild.name}.")
            except discord.HTTPException as e:
                print(f"⚠️ Failed to rename bot role in {guild.name}: {e}")
...
# ========================= RECONNECT GUARD =========================
LOCK_FILE = Path(os.getenv("TMPDIR", "/tmp")) / "bot_last_connect.json"
...
async def check_reconnect_guard():
    ...
    try:
        ...
        connect_count = 1
        first_connect = now
        logging.getLogger(__name__).debug("Reconnect guard read failed: %s", e)
    ...
    try:
        ...
        logging.getLogger(__name__).debug("Reconnect guard write failed: %s", e)
...
async def main():
    ...
    for sig in (signal.SIGINT, signal.SIGTERM):
        try:
            ...
            logging.getLogger(__name__).debug("Signal handler not supported on this platform: %s", sig)
        ...
        try:
            ...
            if bot is not None:
                try:
                    ...
                    logging.getLogger(__name__).debug("bot.close() before restart failed: %s", e)
            ...
            if e.status == 429:
                ...
                await bot.send_critical_alert("Rate Limit", f"Bot is being rate limited by Discord. Waiting {retry_after}s", str(e))
            ...
            traceback.print_exc()
            await bot.send_critical_alert("Login Failure", "Invalid Discord Token", None)
            ...
            traceback.print_exc()
            await bot.send_critical_alert("Bot Crash", "Verification Bot stopped unexpectedly", str(e))
