...
class MSABot(commands.Bot):
    ...
    def _read_lease_payload(self, message):
        ...
        return {
            ...
            "shutdown_old": fields.get("shutdown_old") == "True",
        }
    ...
    def _build_lease_embed(self, payload):
        ...
        embed.add_field(name="shutdown_old", value=str(payload.get("shutdown_old", False)), inline=True)
    ...
    async def _sync_singleton_lease(self, channel):
        ...
        shutdown_old = payload.get("shutdown_old", False)

        if (owner_is_newer and owner_is_alive) or (shutdown_old and owner_id != INSTANCE_ID):
            print(f"🛑 Newer bot instance detected ({owner_id}). Stopping this instance.")
            return True

        # نحن المالكون للـ lease - نكتب shutdown_old فقط لو كان في نسخة أخرى أقدم شغالة
        there_was_older = owner_id is not None and owner_id != INSTANCE_ID and not owner_is_newer
        lease_payload = {
            "instance_id": INSTANCE_ID,
            "started_at": INSTANCE_STARTED_AT,
            "heartbeat_at": now,
            "updated_at": datetime.now(timezone.utc).isoformat(),
            "shutdown_old": there_was_older,
        }
