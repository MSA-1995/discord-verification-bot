...
if not TOKEN:
    print("ERROR: Error: Failed to decrypt DISCORD_TOKEN!")
...
class MSABot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        extensions = ['src.models.verification', 'src.handlers.protection', 'src.core.log_system']
        for ext in extensions:
            try:
                ...
                print(f"ERROR: Failed to load/reload extension {ext}: {e}")

    async def on_ready(self):
        print(f"OK: {self.user} is online and ready!")
        print(f"Connected to Connected to {len(self.guilds)} server(s)")
        print("Protection System: Protection System: ACTIVE (Cogs Mode)")

bot = MSABot()

# تشغيل البوت
print("Starting Starting verification & protection bot...")
try:
    bot.run(TOKEN)
except Exception as e:
